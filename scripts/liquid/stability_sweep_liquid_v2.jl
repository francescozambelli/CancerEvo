# scripts/liquid/stability_sweep_liquid_v2.jl
#
# Adaptive phase-boundary sweep for the liquid-tumor model (v2).
# Improvements over v1:
# - Seed with 500 cells to suppress stochastic drift
# - Run up to 15000 steps to mitigate finite-time artifacts for slowly decaying tumors
# - Target density for invasion success set to 0.5 (since 1.0 is unreachable due to dead cells)
...
const UPPER_LIMIT         = 0.5  # Adjusted from 1.0 to 0.5 since 1.0 is unreachable due to P_death
const MAX_STEPS_STABILITY = 15000 # Increased from 3000 to resolve slow-decay artifacts
const OUTPUT_FILE         = "stability_results_liquid_v2.csv"
const N_SEED_SWEEP        = 500   # Increased from 10 to suppress stochastic drift
...
function simulation_liquid_stability(tiss::LiquidTissue, n_chr_init::Int, n_steps::Int,
    ...
    # Resolve 'Done' cases using running slope logic if necessary,
    # but for simplicity, we assume 15000 steps is enough to separate the regimes.
    # If it's still alive after 15000 steps and density > n_seed / N, it's likely stable.
    if state == "Done"
        if final_density > (N_SEED_SWEEP / N) * 0.5
            state = "Tumor_Max"
        else
            state = "Health"
        end
...
function run_stability_sweep(; n_rmax::Int = 50, dry_run::Bool = false)
    ...
    println("\n═══════════════════════════════════════════════════")
    println("  Direct Bisection Stability Sweep (Liquid v2)")
    println("═══════════════════════════════════════════════════")
