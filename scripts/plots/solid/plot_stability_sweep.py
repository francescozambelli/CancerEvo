...
# Maximum mutation rate to plot is updated to 0.08 in Y space
YMAX = 0.08
...
if adaptive_available:
    ...
    spl = make_smoothing_spline(r_unique, adapt_med_mu, lam=0.0001)
...
if adaptive_available:
    ...
    ax_mpl.fill_between(r_smooth, -0.001, adapt_med_smooth, alpha=0.1, color="#1F77B4")

# Theory critical boundary curve (Blue dashed line)
ax_mpl.plot(theory_rmax_norm, mu_star, color="blue", ls="--", lw=2.5, alpha=0.6, zorder=3, label="Theory (Solid boundary)")
...
if adaptive_available:
    ...
    ax_mpl.scatter(r_unique, adapt_med_mu, color="lightblue", s=80, zorder=10, edgecolor="navy", linewidths=1.5, label="Sim. data")

# Asymptotic saturation
# ax_mpl.axhline(mu_star_sat, color="orange", ls="--", lw=1.5, alpha=0.6, zorder=3)
# ax_mpl.text(XMAX * 0.98, mu_star_sat + 0.001, 
#             r"$\mu_{\infty}^* \approx " + f"{mu_star_sat:.3f}$", 
...
if adaptive_available:
    ...
    fig_plotly.add_trace(go.Scatter(
        ...
        hovertemplate="rmax/r0: %{x:.3f}<br>mu: %{y:.5f}<extra>Sim. data</extra>",
    ))
...
fig_plotly.add_trace(go.Scatter(
    ...
    hovertemplate="rmax/r0: %{x:.3f}<br>mu theory: %{y:.5f}<extra>Theory</extra>",
))
...
fig_plotly.add_hline(
    ...
    annotation_text=f"Solid Saturation &mu; ≈ {mu_star_sat:.3f}",
...
fig_plotly.update_layout(
    title=dict(
        text="Phase Boundary: Critical Mutation Rate &mu;*(r<sub>max</sub>) (Solid Model)",
        font=dict(size=18),
        x=0.5,
    ),
    xaxis=dict(
