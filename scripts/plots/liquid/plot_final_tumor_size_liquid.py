...
def plot_combined():
    ...
    # --- DMU PLOT ---
    vals_dmu, data_size_dmu, means_size_dmu, means_time_dmu = load_data("dmu", v_min=19e-3, v_max=27e-3)
    if vals_dmu is not None:
        ...
        fig_s.tight_layout()
        save_publication_figure(fig_s, "liquid_final_tumor_size_dmu", output_dir=output_dir)
        plt.close(fig_s)

    # --- DR PLOT ---
    vals_dr, data_size_dr, means_size_dr, means_time_dr = load_data("dr", v_min=1e-3, v_max=4e-3)
    if vals_dr is not None:
        ...
        fig_s.tight_layout()
        save_publication_figure(fig_s, "liquid_final_tumor_size_dr", output_dir=output_dir)
        plt.close(fig_s)

    # Save combined pair plot
    fig.tight_layout()
    save_publication_figure(fig, "final_tumor_size_combined_liquid", output_dir=output_dir)
